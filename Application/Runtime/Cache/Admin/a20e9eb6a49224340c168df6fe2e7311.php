            ...
            <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                <div class="widget am-cf">
                    ...
                    <div class="widget-body  am-fr">
                        ...
                        <form action="/index.php/Admin/UserAccount/listAccount" method="get">
                            <div class="am-u-sm-12 am-u-md-4 am-u-lg-4">
                                <div class="am-input-group am-input-group-sm tpl-form-border-form cl-p">
                                    <input type="text" class="am-form-field" name="keyword" placeholder="请输入申请人" value="<?php echo ($keyword); ?>">
                                    <span class="am-input-group-btn">
                                        <button class="am-btn  am-btn-default am-btn-success tpl-table-list-field am-icon-search" type="submit"></button>
                                        <a class="am-btn  am-btn-default am-btn-danger tpl-table-list-field am-icon-refresh" type="button" href="/index.php/Admin/UserAccount/listAccount"></a>
                                    </span>
                                </div>
                            </div>
                        ...
                        <div class="am-u-sm-12">
                            <table width="100%" class="am-table am-table-compact am-table-striped tpl-table-black " id="example-r">
                                ...
                                <?php if(is_array($list)): foreach($list as $key=>$v): ?><tr class="gradeX" title="<?php echo ($v['id']); ?>">
                                        <td><input class="ids" type="checkbox" name="chkbId" value="<?php echo ($v['id']); ?>"></td>
                                        <td><?php echo ($v['id']); ?></td>
                                        <td><?php echo ($v['user_name']); ?></td>
                                        <td>
                                            <?php if($v['type'] == 1): ?>分红提现
                                                <?php else: ?>
                                                余额提现<?php endif; ?>
                                        </td>
                                        <td><?php echo fen_to_yuan($v['money']);?></td>
                                        <td><?php echo ($v['alipay_num']); ?></td>
                                        <td><?php echo ($v['alipay_name']); ?></td>
                                        <td><?php echo ($v['admin_user']); ?></td>
                                        <td>
                                            <?php if($v['admin_time'] == '0'): else: ?>
                                                <?php echo time_format($v['admin_time']); endif; ?>
                                        </td>
                                        <td class="center">
                                            <?php echo time_format($v['create_time']);?>
                                        </td>
                                        <td>
                                            <?php switch($v['state']): case "0": ?>待处理<?php break;?>
                                                <?php case "1": ?>通过<?php break;?>
                                                <?php case "2": ?>驳回<?php break; endswitch;?>
                                        </td>
                                        <td class="f-14">
                                            <div class="tpl-table-black-operation">
                                                <?php if($v['state'] == 0): ?><a title="点击通过" href="<?php echo U('UserAccount/State',array('id'=>$v['id'],'state'=>1));?>">通过</a>
                                                    <a title="点击驳回" href="<?php echo U('UserAccount/State',array('id'=>$v['id'],'state'=>2));?>" class="tpl-table-black-operation-del">驳回</a><?php endif; ?>
                                                <a title="点击查看" href="<?php echo U('UserAccount/AccountInfo',array('id'=>$v['id']));?>">
                                                    <i class="am-icon-eye"></i> 查看
                                                </a>
                                                <a title="点击删除" href="javascript:void(0)" onclick="javascript:recycle(<?php echo ($v['id']); ?>, '确认删除?!此步骤无法恢复!', true)" class="tpl-table-black-operation-del">
