...
<body>
			...
			<div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
				<div class="widget am-cf">
					...
					<div class="widget-body am-fr">
								...
								<div class="am-btn-toolbar">
									<div class="am-btn-group am-btn-group-xs">
										<a type="button" class="am-btn am-btn-default am-btn-success"
										   href="<?php echo U('Banner/editBanner');?>">
						...
						<form action="/index.php/Admin/Banner/listBanner" method="get">
						...
						<div class="am-u-sm-12">
							<table width="100%" class="am-table am-table-compact am-table-striped tpl-table-black">
								...
								<tbody>
								<?php if(is_array($bannerlist)): foreach($bannerlist as $key=>$v): ?><tr class="gradeX">
										<td><input class="ids" type="checkbox" name="chkbId" value="<?php echo ($v['id']); ?>"></td>
										<td><?php echo ($v['id']); ?></td>										
										<td><?php echo ($v['title']); ?></td>
										<td><img src="<?php echo ($v['img_url']); ?>" width="40"></td>
										<td><?php echo ($v['jump_url']); ?></td>
										<td><?php echo ($v['sort']); ?></td>
										<td><?php echo time_format($v['add_time'], 'Y-m-d');?></td>
										<td><?php echo show_dispaly($v['status']);?></td>										
										<td class="f-14">
											<div class="tpl-table-black-operation">
									            <?php if($v['status'] == 0): ?><a title="点击启用" href="javascript:void(0)" onclick="changeDisabled(<?php echo ($v['id']); ?>);">
                                                            启用
                                                        </a>
                                                       <?php else: ?> 
                                                        <a title="点击禁用" href="javascript:void(0)" class="tpl-table-black-operation-del" onclick="changeDisabled('<?php echo ($v["id"]); ?>');">
                                                                禁用
                                                            </a><?php endif; ?>
												<a href="<?php echo U('Banner/editBanner', array('id' => $v['id']));?>">
							...
							<?php if(empty($bannerlist)): ?><h4>aOh! 没有相关内容!</h4><?php endif; ?>
						</div>
	...
	<script type="text/javascript">
        function changeDisabled(id){
            $.ajax({
                url : "<?php echo U('Banner/changeDisabled');?>",
                type : "POST",
                dataType : "json",
                data : {
                    "id" : id,
