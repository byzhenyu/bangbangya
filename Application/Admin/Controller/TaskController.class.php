        ...
        if (IS_POST) {
            ...
            if ($taskModel->create($data, 5) !== false) {
                ...
                if ($data['audit_status'] == 1) {
                    ...
                    $pushRes = D('Common/Push')->push("任务处理结果", $data['user_id'], '任务审核通过', '任务编号: '.$id, '通知类型： 通知', $data['audit_info']);
                    if ($pushRes['status'] == 0) {
                        M()->rollback();
                    ...
                    $pushRes = D('Common/Push')->push("任务处理结果", $data['user_id'], '任务审核未通过', '任务编号: '.$id, '通知类型： 通知', $data['audit_info']);
                    if ($pushRes['status'] == 0) {
                        M()->rollback();
                        $this->ajaxReturn($pushRes);
                    }
